def plot_emergent_time(sequences):
    ...
    for i, seq in enumerate(sequences):
        ...
        ax.scatter(x[-1], y[-1], z[-1], s=200, 
                  c=[plt.cm.twilight(coherence)], marker=(5, 1))
    
    ax.set_xlabel('X (Real Amplitude)')
    ax.set_ylabel('Y (Imaginary Amplitude)')
    ax.set_zlabel('Z (Octave Layer)')
    plt.title('4D COM Structure with Emergent Time from Frequency Waves', pad=20)
    plt.show()